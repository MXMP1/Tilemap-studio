 ...
 *  - КОЛЛИЗИЮ ПРОВЕРЯЕМ ТОЛЬКО ПО КЛЕТКЕ НОГ: walls — непроходимо.
 *  - Голова НЕ проверяет walls: герой (рост 2 клетки) может стоять рядом со стеной
 *    так, что голова находится «перед» блоком стены.
 *  - Слой floor всегда проходим — это «дорога».
 *  - Слой overhead ПРОХОДИМ: герой может «зайти за» куст/крышу,
 *    при этом overhead рисуется ПОВЕРХ героя (скрывает его).
 */
